...
def get_command_arguments():
    ...
    parser.add_argument('--input', '-i', type=str, dest='input', action='store',
                        default='The input file would be a SO_CAL preprocessed text',
...
# gets the configuration settings from the supplied .ini file
    configfile = open('config_files/en_SO_Calc.ini', "r")
...
### load the five kinds of dictionaries
    load_dictionary (io.open (adj_dict_path, encoding = "ISO-8859-1"), adj_dict, c_adj_dict)
    load_dictionary (io.open (adv_dict_path, encoding = "ISO-8859-1"), adv_dict, c_adv_dict)
    load_dictionary (io.open (verb_dict_path, encoding = "ISO-8859-1"), verb_dict, c_verb_dict)
    load_dictionary (io.open (noun_dict_path, encoding = "ISO-8859-1"), noun_dict, c_noun_dict)
    load_dictionary (io.open (int_dict_path, encoding = "ISO-8859-1"), int_dict, c_int_dict)
    if extra_dict_path:
        load_extra_dict(io.open (extra_dict_path, encoding = "ISO-8859-1"))
    if simple_SO:
...
def fill_text_and_weights(line):
...
def run(text_input):
    ...
    global word_counts
    word_counts = [{},{},{},{}]
    fill_text_and_weights(text_input)

    if fix_cap_tags:
        fix_all_caps()

    print "before it all began", text_SO, SO_counter, len(text)
    ...
    if SO_counter > 0:
        text_SO = text_SO / SO_counter

    print "inside func", text_SO, SO_counter
